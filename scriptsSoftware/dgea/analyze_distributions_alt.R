...
option_list <- list(
    ...
    make_option(
        ...
        type="numeric",
        default=21,
        help="Width of plot canvas in inches. The plotting canvas contains 3 individual juxtaposed plots. Default: %default.",
        metavar="float"
    ),
    make_option(
        "--plot-height",
        ...
        type="numeric",
        default=7,
        help="Height of plot canvas in inches. Default: %default.",
        metavar="float"
    ),
    make_option(
        c("-h", "--help"),
...
plot.dist <- function(
    ...
    if ( ! is.null(subset.data) ) {
        ...
        subsets <- lapply(names(subset.data), function(name) {
            ...
            if ( length(ls$dat.smp) >= min.feat.per.set && length(ls$dat.pop) >= min.feat.per.set ) {
                ...
                ls$t.test <- t.test(ls$dat.pop, ls$dat.smp)
                ls$t <- ls$t.test$statistic
                ls$t.p <- ls$t.test$p.value
    ...
    par(mar=mar.plot)
    plot(cdf, main="", xlab=xlab, ylab="Cumulative fraction", do.points=do.points, lwd=lwd)
    ...
    par(mar=mar.plot)
    plot(cdf.abs, main=NULL, xlab=xlab, ylab="Cumulative fraction", do.points=do.points, lwd=lwd)
    if ( ! is.null(vertical.bar) && vertical.bar > 0 ) abline(v=vertical.bar, lty=2)
    if ( cdf.median ) abline(h=0.5, lty=2)

    # Plot density
    par(mar=mar.plot)
    plot(dens, main="", xlab=xlab, ylab="Density", lwd=lwd)
    if ( ! is.null(vertical.bar) ) abline(v=vertical.bar, lty=2)
    ...
    if ( ! is.null(subset.data) ) {
        ...
        for ( subset in subsets ) {
            ...
            # Build output filename
            out.file <- file.path(out.dir, paste(prefix, subset$desc.safe, suffix, sep=sep))
            ...
            # Plot cumulative distribution
            par(mar=mar.plot)
            plot(subset$cdf.pop, main="", xlab=xlab, ylab="Cumulative fraction", col=4, lwd=lwd, do.points=do.points)
            plot(subset$cdf.smp, do.points=do.points.subsets, col=2, pch=20, cex=0.75, lwd=lwd, add=TRUE)
            ...
            legend("bottomright", inset=c(0.02, 0.05), legend=c(subset$desc, "remaining"), col=c(2, 4), lty=1, bty="n")
            if ( ! is.null(vertical.bar) ) abline(v=vertical.bar, lty=2)
            if ( cdf.median ) abline(h=0.5, lty=2)

            # Plot cumulative distribution (absolute values)
            par(mar=mar.plot)
            plot(subset$cdf.pop.abs, main="", xlab=xlab, ylab="Cumulative fraction", col=4, lwd=lwd, do.points=do.points)
            plot(subset$cdf.smp.abs, do.points=do.points.subsets, col=2, pch=20, cex=0.75, lwd=lwd, add=TRUE)
            ...
            plot(subset$dens.pop, main="", xlab=xlab, ylab="Density", ylim=ylim, col=4, lwd=lwd)
            lines(subset$dens.smp, col=2, lwd=lwd)
            t <- paste("t", signif(subset$t, digits=3), sep=" = ")
            t.p <- paste("P", signif(subset$t.p, digits=3), sep=" = ")
            legend <- c("Student's t-test", t, t.p)
            legend("topleft", inset=c(0, 0.02), legend=legend, bty="n")
            legend("topright", inset=c(0.02, 0.02), legend=c(subset$desc, "remaining"), col=c(2, 4), lty=1, bty="n")
            if ( ! is.null(vertical.bar) ) abline(v=vertical.bar, lty=2)
...
all.results <- lapply(colnames(dat), function(name) {
    ...
    # Write log
    if ( verb ) cat("Plotting distributions for column ", name, "...\n", sep="'")
...
mt.t <- sapply(all.results, lapply, "[[", "t")
mt.t.p <- sapply(all.results, lapply, "[[", "t.p")
...
out.file.t <- file.path(out.dir, paste(run.id, "statistics", "t_test", "t", "tsv", sep="."))
out.file.t.p <- file.path(out.dir, paste(run.id, "statistics", "t_test", "p_values", "tsv", sep="."))
...
write.table(mt.t, out.file.t, col.names=TRUE, row.names=TRUE, quote=FALSE, sep="\t")
write.table(mt.t.p, out.file.t.p, col.names=TRUE, row.names=TRUE, quote=FALSE, sep="\t")
