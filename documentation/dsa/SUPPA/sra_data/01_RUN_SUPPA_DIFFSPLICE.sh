...
outDirRoot="${root}/analyzedData/dsa/SUPPA/sra_data"
tmpDir="${root}/.tmp/analyzedData/dsa/SUPPA/sra_data"
...
if [ ! -f "$psi" ]; then echo "WHAT?!!"; fi
        ...
        IFS=$'\n'
                    ...
                    #significanceCalculator.py --method "$SUPPA_method" --ioe "$ioe" --psi "$refPsi" "$queryPsi" --expression-file "$refTpm" "$queryTpm" --area "$SUPPA_area" -gc --output "$SUPPA_outPrefix" &>> "$logFile"
