...
# Build output filename
out_file="${outDir}/${mergedPrefix}${outputSuffixDPSI}"

# Log status
echo "Delta PSI: Merging all data tables and writing to file '$out_file'..." >> "$logFile"

# Merge data tables
"${mergeScript}" --input-directory "$outDir" --output-table "$out_file" --glob "*$outputSuffixDPSI" --verbose &>> "$logFile"
