        ...
        for rType in "${types[@]}"; do
            ...
            for qType in "${types[@]}"; do
                ...
                if [ ! "$ref" == "$query" ] ; then
                    ...
                    # Run SUPPA diffSplice
                    echo "significanceCalculator.py --method \"$SUPPA_method\" --input \"$ioe\" --psi \"$refPsi\" \"$queryPsi\" --tpm \"$refTpm\" \"$queryTpm\" --area \"$SUPPA_area\" -gc --output \"$SUPPA_outPrefix\"" &>> "$logFile"
